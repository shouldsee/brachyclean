...
if quick==1:
    ...
    mdl0 = pyjob.vmfMixture__anneal(dfc, start, end,K=30,
                                    nIter=200)
if quick==-1:
    mdl0 = pyext.readBaseFile('results/0315__cluster/mdl0.npy').tolist()
else:
...
pyutil.printlines(sutil.tidyBd(cluc.query('clu==5')).index,'temp.txt')
res.query('clu==5')[['Best-hit-arabi-name','score']].dropna().to_csv('temp.txt',sep='\t',index=0)
