...
def prefilter(dfc):
    CUTOFF=np.log2(20+1)
    NDROP = 3
    dfc =dfc.copy().setDF(np.clip(dfc.values,CUTOFF,None))
    toDrop = dfc.index[ np.sum(dfc.values > CUTOFF,axis=1) <= NDROP]
    dfc = dfc.drop(index=toDrop)
    
#     dfc = dfc.apply(pyutil.log2p1)
    
    dfc.qc_Avg();
#     dfc = dfc.reindex(dfc.summary.query('SD!=0').index)    
    dfc = sutil.stdNorm(dfc,
#                         meanNorm=1
                       )
